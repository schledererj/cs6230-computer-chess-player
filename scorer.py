...
class Scorer:
	...
	def _is_end_game(board: Board) -> bool:
		...
		_white_pieces = [_piece_map[x].symbol().upper() for x in filter(lambda y: _piece_map[y].symbol().isupper(), _piece_map)]
		_black_pieces = [_piece_map[x].symbol().upper() for x in filter(lambda y: _piece_map[y].symbol().islower(), _piece_map)]
